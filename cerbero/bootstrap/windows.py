...
class WindowsBootstrapper(BootstrapperBase):
    ...
    def _install_python_sdk(self):
        url = 'https://github.com/Mingyiz/cerbero-archive/raw/master/windows-external-sdk.tar.xz'

        ### FIXME : MOVE OVER REPOSITORY TO STANDARD ROOT
        old_sdk_git_root = 'git://anongit.freedesktop.org/gstreamer-sdk'
        m.action(_("Installing Python headers"))
        tmp_dir = tempfile.mkdtemp()
        path = os.path.join(tmp_dir, 'windows-external-sdk.tar.xz')
        shell.download( url , path, False, check_cert=False)
    ...
    def _install_nasm(self):
        ...
        shell.download( url , tarball, False, check_cert=False)
